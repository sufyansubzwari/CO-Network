...
Mutation.user = async (root, { user }, context) => {
  ...
  if (profile.knowledge && profile.knowledge.languages)
    profile.knowledge.languages = await Tags.service.normalizeTagsWithLevels(
      profile.knowledge.languages,
      oldUser &&
      oldUser.profile &&
      oldUser.profile.knowledge &&
      oldUser.profile.knowledge.languages
        ? oldUser.profile.knowledge.languages
        : []
    );
  if (profile.knowledge && profile.knowledge.curiosity)
    profile.knowledge.curiosity = await Tags.service.normalizeTagsWithLevels(
      profile.knowledge.curiosity,
      oldUser.profile &&
      oldUser.profile.knowledge &&
      oldUser.profile.knowledge.curiosity
        ? oldUser.profile.knowledge.curiosity
        : []
    );
  if (profile.professional && profile.professional.industry)
    profile.professional.industry = await Tags.service.normalizeTags(
      profile.professional.industry,
      oldUser.profile &&
      oldUser.profile.professional &&
      oldUser.profile.professional.industry
        ? oldUser.profile.professional.industry
        : []
    );
  if (profile.speaker && profile.speaker.otherpreferred)
    profile.speaker.otherpreferred = await Tags.service.normalizeTags(
      profile.speaker.otherpreferred,
      oldUser.profile &&
      oldUser.profile.speaker &&
      oldUser.profile.speaker.otherpreferred
        ? oldUser.profile.speaker.otherpreferred
        : []
    );
  if (profile.speaker && profile.speaker.otherlooking)
    profile.speaker.otherlooking = await Tags.service.normalizeTags(
      profile.speaker.otherlooking,
      oldUser.profile &&
      oldUser.profile.speaker &&
      oldUser.profile.speaker.otherlooking
        ? oldUser.profile.speaker.otherlooking
        : []
    );
  if (profile.speaker && profile.speaker.topic)
    profile.speaker.topic = await Tags.service.normalizeTags(
      profile.speaker.topic,
      oldUser.profile &&
      oldUser.profile.speaker &&
      oldUser.profile.speaker.topic
        ? oldUser.profile.speaker.topic
        : []
    );
  ...
  await Achievement.deleteAchievement({ owner: inserted._id });
