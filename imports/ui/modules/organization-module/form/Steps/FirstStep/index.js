...
class FirstStep extends React.Component {
  ...
  handleLink = (service, updateIdentitiesOrg) => {
    Authorization.loginOrganization(service, profile =>
      this.handleProfile(profile, service, updateIdentitiesOrg)
    );
  };
  ...
  handleUnlink = (element, updateIdentitiesOrg) => {
    ...
    let accounts =
      this.state.organization && this.state.organization.identities;
  ...
  getAccountService = service => {
    return (
      ...
      this.state.organization.identities &&
      this.state.organization.identities.length > 0 &&
      this.state.organization.identities.filter(
        element => element.provider === service
      )
    );
  };
                    ...
                    return accounts.map((element, index) => {
                      ...
                      return (
                        <SocialButton
                          ...
                          onClose={() =>
                            this.handleUnlink(element, updateIdentitiesOrg)
                          }
                        />
                      );
                    });
