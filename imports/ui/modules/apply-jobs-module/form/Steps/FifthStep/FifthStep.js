            ...
            <Layout rowGap={"25px"}>
                <Query query={achievementsQuery} variables={{ id: this.state.apply.userId  }}>
                    {({ loading, error, data }) => {
                        ...
                        let achievements = data && data.user && data.user.profile && data.user.profile.achievements;
                        if(achievements && !this.state.apply.achievements)
                            this.handleChange(achievements);
                        return (
                <Achievements
                    onChange={this.handleChange}
                    achievements={this.state.apply.achievements && this.state.apply.achievements.length && this.state.apply.achievements.map(item => ({...item}))}
                />
                        );
                    }}
                </Query>
