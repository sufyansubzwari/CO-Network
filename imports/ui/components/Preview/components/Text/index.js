...
class Text extends React.Component {
  ...
  handleRef = ref => {
    ...
    if (this.state.lines === 100)
      if (
        this.textRef &&
        Math.floor(this.textRef.clientHeight / 22) > this.props.cutLines
      ) {
  ...
  render() {
    const extras =
      this.props.extraTexts &&
      this.props.extraTexts.length > 0 &&
      this.props.extraTexts.some(item => item && item.text && item.text !== "");
    return (
      <Container>
        {this.props.header ? (
        ...
        {this.props.text ? (
          <Container>
            ...
            {(!this.state.showMore || this.props.displayExtraTexts) &&
              this.props.extraTexts &&
              this.props.extraTexts.length > 0 &&
              this.props.extraTexts.map(
                (item, index) =>
            ...
            {(this.props.cutText && this.state.renderMore) ||
            (this.props.showMore && extras) ? (
              this.state.showMore ? (
                <SSpan onClick={this.handleMore}>More...</SSpan>
              ) : (
                <SSpan onClick={this.handleMore}>Less...</SSpan>
              )
            ) : null}
          </Container>
        ) : this.props.children ? (
