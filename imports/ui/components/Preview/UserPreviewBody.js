...
class UserPreviewBody extends React.Component {
    ...
    render() {
        ...
        //tags
        let languages = this.state.user.knowledge.languages.map(lang => ({...lang, active: true}))
        let curious = this.state.user.knowledge.curiosity.map(cur => ({...cur, active: true}))
        let industry = this.state.user.professional.industry.map(ind => ({name: ind, active: true}))
        let otherlook = this.state.user.speaker.otherlooking.map(look => ({...look, active: true}))
        let otherpre = this.state.user.speaker.otherpreferred.map(pre => ({...pre, active: true}))
        let topics = this.state.user.speaker.topic.map(top => ({...top, active: true}))

        //checkboxes
        let lookingfor = this.state.user.knowledge.lookingFor.map(look => <div>{look.label}</div>)
        let jobtype = this.state.user.professional.jobType.map(job => <div>{job.label}</div>)
        let lookingforS = this.state.user.speaker.lookingFor.map(look => <div>{look.label}</div>)
        let preferred = this.state.user.speaker.stage.map(stage => <div>{stage.label}</div>)

        return (
            <Layout rowGap={'15px'}>
                <Title text={this.state.user.info.name + " " + this.state.user.info.lastName}/>
                <Location text={this.state.user.info.location.address.toUpperCase()}/>
                {this.state.user.info.email !== '' ? <Text header={'Email'} text={this.state.user.info.email}/> : null}
                <Social social={socials}
                        links={[{link: this.state.user.info.website, website: this.state.user.info.website}]}/>
                <Container>
                ...
                {this.state.user.aboutMe.yourPassion !== '' ?
                    <Text header={'About Me | Passion'} text={this.state.user.aboutMe.yourPassion}/> : null}
                {this.state.user.aboutMe.existingProblem !== '' ?
                    <Text header={'What is the most exciting real world problem you want to solve?'}
                          text={this.state.user.aboutMe.existingProblem}/> : null}
                {this.state.user.aboutMe.steps !== '' ?
                    <Text header={'What steps have you already taken towards achieving this mission?'}
                          text={this.state.user.aboutMe.steps}/> : null}
                ...
                <Layout templateColumns={3}>
                    ...
                    {this.state.user.professional.salaryRange.min !== "" || this.state.user.professional.salaryRange.max !== "" ?
