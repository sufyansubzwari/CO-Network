pipeline {
	...
	stages {
        ...
        stage('Deploy to jenkins-github') {
            when {expression { env.BRANCH_NAME == 'feature/jenkinsfile-github-integration' }}
            steps {
                sh '''
	            #!/bin/bash
	    ...
	    stage('Deploy Dev') {
	        ...
	        steps {
	            ...
	            #!/bin/bash
	            echo Copying code to GitHub
	            sh connect_to_ml_github.sh
	            cp ./* ../scripts/ml/CO-Network
	            sh push_ml_to_github.sh
	            rm -rf ../scripts/ml/CO-Network
