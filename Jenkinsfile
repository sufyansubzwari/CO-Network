	...
	stages {
	    stage('Deploy Dev') {
	        ...
	        steps {
	            ...
	            sudo ssh -i /home/ubuntu/PSE ubuntu@50.19.25.171 'aws ssm get-parameter --name "/secrets/cdt/mlsociety-new-design" --region us-east-1 --query 'Parameter.{V:Value}' --output text > settings; METEOR_SETTINGS=`jq -n -f settings`; sudo $(aws ecr get-login --no-include-email --region us-east-1); sudo docker stop mlsociety-new-design-in-dev; sudo docker rm mlsociety-new-design-in-dev; sudo docker pull 843249697839.dkr.ecr.us-east-1.amazonaws.com/btech/mlsociety-new-design:dev; sudo docker run -d --name mlsociety-new-design-in-dev -e MONGO_URL="mongodb://co-network:i6QJc31zRLvc48Fg@co-network-shard-00-00-d377d.mongodb.net:27017,co-network-shard-00-01-d377d.mongodb.net:27017,co-network-shard-00-02-d377d.mongodb.net:27017/devops-test2?authSource=admin&ssl=true&replicaSet=CO-NETWORK-shard-0" -e METEOR_SETTINGS="$METEOR_SETTINGS" -e ROOT_URL=https://staging.mlsociety-new-design.tek-task.com -p 8082:3000 843249697839.dkr.ecr.us-east-1.amazonaws.com/btech/mlsociety-new-design:dev'
	            '''
